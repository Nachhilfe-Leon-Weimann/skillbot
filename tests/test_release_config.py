...
(The other ``extra-files`` entry, ``uv.lock``, has its own backstop: CI installs with ``uv sync --locked``.)
"""
...
PINNED_IMAGE_LINE = re.compile(
    r"^\s+image: ghcr\.io/nachhilfe-leon-weimann/skillbot:v(?P<version>\d+\.\d+\.\d+) # x-release-please-version$"
)
...
def _extra_files() -> list[dict[str, str]]:
...
def test_release_please_rewrites_compose_yml() -> None:
    assert {"type": "generic", "path": "compose.yml"} in _extra_files()


def test_release_please_rewrites_the_lockfile() -> None:
    assert {
        "type": "toml",
        "path": "uv.lock",
        "jsonpath": "$.package[?(@.name.value=='skillbot')].version",
    } in _extra_files()
